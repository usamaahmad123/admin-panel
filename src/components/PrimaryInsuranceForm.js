...
const PrimaryInsuranceForm = ({ onSubmittion, SnacbarViewer, id, setPrimaryInfo, PrimaryRef }) => {
      ...
      getInsurance(id).then((res) => {
        if (res.success) {
          for (let i = 0; i < res.data.length; i += 1) {
            if (res.data[i].insurance_type === 'Primary Insurance') {
              setPrimaryInfo(true);
              insuranceExists = true;
              formik.setFieldValue(
                'insurance_type',
                res.data[i].insurance_type || 'Primary Insurance'
              );
...
PrimaryInsuranceForm.propTypes = {
  onSubmittion: PropTypes.any,
  SnacbarViewer: PropTypes.any,
  id: PropTypes.any,
  setPrimaryInfo: PropTypes.any,
  PrimaryRef: PropTypes.any
};
export default PrimaryInsuranceForm;
