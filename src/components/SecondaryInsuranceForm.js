...
const SecondaryInsuranceForm = ({
      ...
      getInsurance(id).then((res) => {
        if (res.success) {
          for (let i = 0; i < res.data.length; i += 1) {
            if (res.data[i].insurance_type === 'Secondary Insurance') {
              setSecondaryInfo(true);
              insuranceExists = true;
              formik.setFieldValue(
                'insurance_type',
                res.data[i].insurance_type || 'Secondary Insurance'
              );
  ...
  const formik = useFormik({
    initialValues: {
      patient_id: id,
      insurance_type: 'Secondary Insurance',
      se_address: '',
      se_city: '',
      se_street: '',
      se_zip: '',
            ...
            <AccordionDetails>
              <Stack fullWidth spacing={3}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                  <TextField
                    fullWidth
                    label="Secondary insurance provider name"
                    size="small"
                    InputProps={{
                      classes: {
...
SecondaryInsuranceForm.propTypes = {
  onSubmittion: PropTypes.any,
  SnacbarViewer: PropTypes.any,
  id: PropTypes.any,
  setSecondaryInfo: PropTypes.any,
  SecondaryRef: PropTypes.any
};
export default SecondaryInsuranceForm;
